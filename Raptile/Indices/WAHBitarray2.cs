...
namespace Raptile.Indices
{
    ...
    /// http://www.codeproject.com/Articles/214997/Word-Aligned-Hybrid-WAH-Compression-for-BitArrays
    /// </summary>
    internal class WAHBitArray
    {
        ...
        public WAHBitArray(WAHType wahType, uint[] ints)
        {
            _usingIndexes = false;
            switch (wahType)
            {
                case WAHType.Compressed:
                    _compressed = ints;
                    break;
                case WAHType.Uncompressed:
                    _uncompressed = ints;
                    break;
                case WAHType.Indexes:
        ...
        public WAHBitArray And(WAHBitArray op)
        {
            ...
            return new WAHBitArray(WAHType.Uncompressed, ints);
        }
        ...
        public IEnumerable<int> GetBitIndexes(bool ones)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    if (_uncompressed[i] > 0 && ones)
                    {
                        for (var j = 0; j < 32; j++)
                        {
                            bool b = HighOrderBitGet((i << 5) + j);
                            if (b)
                                yield return (i << 5) + j;
                        }
                    }
        ...
        private void internalSet(int index, bool val)
        {
            isDirty = true;
            int pointer = index >> 5;
            uint mask = (uint)1 << (31 - (index % 32)); // high order bit set
        ...
        private bool HighOrderBitGet(int index)
        {
            var pointer = index >> 5;
            uint mask = (uint)1 << (31 - (index % 32)); // high order bit get
        ...
        private uint Take31Bits(int index)
        {
            ...
            long l1 = _uncompressed[pointer];
            pointer++;
            if (pointer < _uncompressed.Length)
                l2 = _uncompressed[pointer];

            long l = (l1 << 32) + l2;
            long ret = (l >> (33 - off)) & 0x7fffffff;
        ...
        private void Uncompress()
        {
            ...
            foreach (uint ci in _compressed)
            {
                ...
                {
                    Write31Bits(list, index, ci & 0x7fffffff);
                    index += 31;
                }
                else
                {
        ...
        private static void FixLengths(uint[] ar1, uint[] ar2)
        {
            int ic = ar1.Length;
            int uc = ar2.Length;
            if (ic > uc)
        ...
        private static void WriteBits(List<uint> list, int index, uint count)
        {
            ResizeAsNeeded(list, index);
            ...
            if (bit > 0)
            {
                list[pointer] |= ~(0xffffffff << (32 - bit));
                cc -= (32 - bit);
            }
            else
            {
